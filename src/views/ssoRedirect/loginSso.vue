<template>
  <el-container :class="['basic_layout_container']">
    <div class="backBox">
      ...
      <div class="loginCard" v-if="isFirstLogin">
        <div class="title">
      ...
      <!-- 底部开始 -->
      <div id="footer">
        <span class="pull-right">© 2022 Winning Health 版权所有</span>
      </div>
    </div>
  </el-container>
</template>
...
export default {
  ...
  created() {
    ...
    if (this.originalParam.ptflag) {
      // 非首次使用单点登录系统直接进入患者360床位卡页,否则要输入用户名、密码登录后进行SSO注册。
      if (this.originalParam.loginflag === '2') {
        this.isFirstLogin = false;
        this.handleLogin();
      } else { this.isFirstLogin = true; }
    }
  },
  mounted() {
  ...
  methods: {
    initRequestParams(url, originalParam) {
      ...
      if (this.originalParam.portalurl) {
        localStorage.setItem('portalurl', this.originalParam.portalurl);
      }
    },
    // 勾选保存账号
    rememberUsername() {
    ...
    // 点击登录调用vuex
    async handleLogin() {
      if (this.originalParam.loginflag !== '2' && (this.username.trim() === "" || this.password.trim() === "" || this.chooseInstitution === '')) {
        this.$message({
          showClose: true,
      ...
      this.originalParam.usercode = this.username.trim() ? Base64.encode(this.username.trim()) : "";
      this.originalParam.password = this.password.trim() ? Base64.encode(this.password.trim()) : "";
      this.originalParam.yljgdm = this.chooseInstitution ? Base64.encode(this.chooseInstitution) : "";

      this.$api
        .loginFromSSO(this.originalParam)
        .then((data) => {
          if (data.message === 'SUCCESS') {
            this.$store.commit("SET_TOKEN", data.data.token);
            this.$storage.set("username", this.originalParam.loginid);
            const userMessage = {
              ...
              passwordExpiryDate: data.data.passwordExpiryDate,
