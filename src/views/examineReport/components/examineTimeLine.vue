        ...
        >
          ...
          >
            ...
            <div class="mainContent">
              ...
              >
                <span @click.stop="getReportContent(items)">
                  <span class="error" v-if="items.ycbz">*</span>
                  <span class="times">
                    {{ items.inspectionAt || "-" }}
                  </span>
                  <span class="contents">{{
                    items.medtechRptTitle || "-"
                  }}</span>
                </span>
              </p>
            </div>
...
export default {
  ...
  mounted() {
    // 拷贝一份时间线数据
    this.newTimeLineData = JSON.parse(JSON.stringify(this.timeLineData));
    this.getSingleLineData();
    // 初始化拿到第一条的颜色
    this.currentIndex = this.timeLineData.content[0].medtechRptId;
  ...
  methods: {
    ...
    async getSingleLineData() {
      try {
        ...
        const data = await this.$api.getLisReportList(params);
        ...
        // 加了一个hospitalId
        const sendToReportMsg = {
          ...data.content[0],
          hospitalSoid: this.timeLineData.content[0].hospitalSoid,
        };
        // 先传一次timeLineContent的第一条数据渲染右侧上面内容
        this.$emit("timeLineContentClick", sendToReportMsg);
    ...
    async getlineData(info) {
      console.log("卡片头部", info, info.hospitalSoid);
      // 再传一个hospitalId到兄弟组件
      this.$emit("timeLineContentClick", info);
      try {
        const params = {
          bizRoleId: this.patientInfo.content[0].bizRoleId,
        ...
        this.currentIndex = data.content[0].medtechRptId;
        console.log("info.hospitalSoid", info.hospitalSoid); // 目前陈秀洁两条数据hospitalId一样的
        // 点击头部的时候，默认右侧调第一行的数据
        // 加了一个hospitalId
        const sendToReportMsg = {
          ...data.content[0],
        ...
        this.getReportContent(sendToReportMsg);
      } catch (error) {
        console.log(error);
      }
    },
    ...
    // 不同报告的hospitalId是否都取自于vuex
    getReportContent(item) {
      const sendToReportMsg = {
        ...item,
        hospitalSoid: this.infos.hospitalSoid,
      };
      console.log(sendToReportMsg, "sendToReportMsg");
    ...
    async toggleCard(index) {
      try {
        ...
        // 切换不同卡片的时候 是否也默认切换表头？这样能点击更灵活 todo...
